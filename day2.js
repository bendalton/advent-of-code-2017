...
let inputSheet = `157	564	120	495	194	520	510	618	244	443	471	473	612	149	506	138
...
/**
 * the sheet's checksum is the sum of the difference between each row's largest and smallest values
 * e.g.,
 * 1 2 3 4 would be 4 - 1 = 3
 * 
 * @param {String} inputSheet
 */
function calculateChecksum(inputSheet){
  const rows = inputSheet.split("\n")
  let checksum = 0;
  rows.forEach((row)=>{
    const vals = row.split("	").map((n) => parseInt(n)).sort((a, b) => a > b ? 1 : a < b ? -1 : 0)
    checksum += vals.pop() - vals.shift();
  })
  return checksum;
}

console.log("Problem 1", calculateChecksum(inputSheet));
benchmark(() => calculateChecksum(inputSheet));


/**
 * the sheet's checksum is the sum of the quotient of the pair of evenly divisible values in a row
 * e.g., 
 * 1 2 3 4 would be 4 / 2 = 2
 * 
 * @param {String} inputSheet 
 */
function calculateChecksumDivisible(inputSheet) {
  const rows = inputSheet.split("\n")
  let checksum = 0;
  rows.forEach((row) => {
    const vals = row.split("	").map((n) => parseInt(n)) // array of integers
    let rowProduct = -1;
    while(rowProduct === -1){ // go until we find a match
      const a = vals.shift(); // compare 1 at a time, shortening the array
      vals.forEach((b)=>{ // look at the REMAINING items in the array
        if(a%b === 0){ // if a is evenly divisible by b
          rowProduct = a/b;
        }else if(b%a === 0){ // if b is evenely divisible by a
          rowProduct = b/a
        }
      })
    }
...
console.log("Problem 2", calculateChecksumDivisible(inputSheet))
benchmark(()=>calculateChecksumDivisible(inputSheet));
